...
const UpdateStatusConfirmationModal = ({
  ...
  const handleConfirm = async () => {
    ...
    const loadingToast = toast.loading(
      `Mengubah status kursi ${seat?.label}...`
    )
    try {
      await onConfirm()
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div className="mt-2">
              <p className="text-sm text-gray-500">
                ...
                <strong className="font-mono bg-gray-200 px-1 rounded">
                  {seat.label}
                </strong>{' '}
                menjadi{' '}
                <strong className="text-red-600">Dipesan (Booked)</strong>? Aksi
                ini untuk penjualan tiket offline.
              </p>
            </div>
          </div>
        </div>
