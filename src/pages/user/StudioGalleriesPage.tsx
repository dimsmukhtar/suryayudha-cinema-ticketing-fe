...
const StudioCard = ({
  ...
  >
    <img
      src={
        studio.galleries[0]?.photo_url ||
        'https://placehold.co/600x400/1a202c/ffffff?text=Studio'
      }
      alt={studio.name}
...
const StudioGalleriesPage = () => {
  ...
  useEffect(() => {
    const fetchAndProcessData = async () => {
      setIsLoading(true)
      try {
        const [studiosData, photosData] = await Promise.all([
          getAllStudios(),
          getAllStudioPhotos()
        ])
  ...
  if (isLoading)
    return <div className="text-white text-center py-20">Memuat Studio...</div>
